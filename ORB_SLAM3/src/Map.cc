...
{
    int nMPWithoutObs = 0;
    // Iterate over a copy: EraseObservation() can call SetBadFlag() -> EraseMapPoint(this), invalidating mspMapPoints iterators
    std::set<MapPoint*> copyMapPoints = mspMapPoints;
    for(MapPoint* pMPi : copyMapPoints)
    {
        if(!pMPi || pMPi->isBad())
            continue;
