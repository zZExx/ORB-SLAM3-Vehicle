...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
               const bool bUseViewer, const int initFr, const string &strSequence, const bool bApplyWheelFromYaml):
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    mpTracker = new Tracking(this, mpVocabulary, mpFrameDrawer, mpMapDrawer,
                             mpAtlas, mpKeyFrameDatabase, strSettingsFile, mSensor, settings_, strSequence, bApplyWheelFromYaml);
...
void System::Shutdown()
{
    ...
    // Wait until all thread have effectively stopped (required before SaveAtlas to avoid segfault in PreSave)
    int waitCount = 0;
    const int waitLimit = 6000;  // 30 s at 5ms
    while((!mpLocalMapper->isFinished() || !mpLoopCloser->isFinished() || mpLoopCloser->isRunningGBA()) && waitCount < waitLimit)
    {
    ...
    if(waitCount >= waitLimit)
        cout << "Shutdown: wait for threads timed out" << endl;
    // Join threads so they are fully exited before we touch the atlas
...
void System::SaveAtlasNow()
{
    if(mStrSaveAtlasToFile.empty())
    {
        cout << "[SaveAtlasNow] mStrSaveAtlasToFile is empty, skip" << endl;
        return;
    }
    try {
        Verbose::PrintMess("Atlas saving to file " + mStrSaveAtlasToFile, Verbose::VERBOSITY_NORMAL);
        SaveAtlas(FileType::BINARY_FILE);
        cout << "[SaveAtlasNow] done: " << mStrSaveAtlasToFile << ".osa" << endl;
    } catch (const std::exception& e) {
        cerr << "[SaveAtlasNow] exception: " << e.what() << endl;
        throw;
    } catch (...) {
        cerr << "[SaveAtlasNow] unknown exception" << endl;
        throw;
    }
}
...
cv::Mat System::GetTrackedImage(float imageScale)
{
    if(!mpFrameDrawer)
        return cv::Mat();
...
void System::SaveAtlas(int type){
    if(!mStrSaveAtlasToFile.empty())
    {
        cout << "[SaveAtlas] PreSave..." << endl;
        mpAtlas->PreSave();

        string pathSaveFileName = "./";
        pathSaveFileName = pathSaveFileName.append(mStrSaveAtlasToFile);
        pathSaveFileName = pathSaveFileName.append(".osa");
        cout << "[SaveAtlas] path: " << pathSaveFileName << endl;
        ...
        else if(type == BINARY_FILE) // File binary
        {
            ...
            std::ofstream ofs(pathSaveFileName, std::ios::binary);
            if(!ofs.is_open()) {
                cerr << "[SaveAtlas] failed to open file: " << pathSaveFileName << endl;
                return;
            }
