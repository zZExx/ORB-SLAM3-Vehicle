            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        ...
                        {
                            const float imuInitMaxTime = mpTracker ? mpTracker->GetImuInitMaxTime() : 10.0f;
                            const float imuInitMinMotion = mpTracker ? mpTracker->GetImuInitMinMotion() : 0.02f;
                            if((mTinit<imuInitMaxTime) && (dist<imuInitMinMotion))
                            {
...
{
    // 从共视图中取邻居关键帧：后续只在这些帧之间做匹配/三角化
    int nn = 10;
    // 单目通常需要更多邻居来提高可三角化机会
    if(mbMonocular)
        nn=30;
    // 关键点：这里返回的是“按共视权重排序”的前nn个关键帧（权重=共享MapPoint数量）
    // 共视关系由KeyFrame::UpdateConnections维护，因此这一步决定了建点配对集合的质量
    vector<KeyFrame*> vpNeighKFs = mpCurrentKeyFrame->GetBestCovisibilityKeyFrames(nn);

    if (mbInertial)
    {
        // 惯导模式再补充时间邻居：与共视邻居互补，提升连续运动场景的建点稳定性
    ...
    // 主流程：当前关键帧与每个邻居做“匹配 -> 三角化/反投影 -> 几何筛选 -> 入图”
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        // 基线门限：单目下用 baseline/depth 过滤纯旋转或弱平移场景，避免退化
        ...
        // 通过极线约束搜索可三角化匹配；recently lost时允许更粗匹配策略
        ...
        for(int ikp=0; ikp<nmatches; ikp++)
        {
            ...
            // 先看视差：视差足够则双目三角化，否则尝试用单帧双目反投影补救
            ...
            // 重投影误差筛选（第1帧）：单目用2D误差，双目额外约束右目误差
