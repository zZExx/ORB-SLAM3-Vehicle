...
Tracking::Tracking(System *pSys, ORBVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
    ...
    mInitStage(MONO_ONLY), mInitStartTime(-1.0), mInitTimeout(3.0), mInitMinKeyFrames(8), mInitStableFramesRequired(8), mInitStableFrameCount(0),
    mbInitMonoLogged(false), mbInitVioLogged(false),
    mpImuInitializerHelper(new IMUInitializer(pAtlas)),
    ...
    mnResetMinKeyFrames(10)
{
    ...
    if(settings){
        ...
        // Optional init-stage tuning for monocular-inertial bring-up.
        cv::FileNode initTimeoutNode = fSettings["Init.Timeout"];
        if(!initTimeoutNode.empty() && initTimeoutNode.isReal())
        {
            mInitTimeout = initTimeoutNode.real();
        }
        else if(!initTimeoutNode.empty() && initTimeoutNode.isInt())
        {
            mInitTimeout = static_cast<double>(initTimeoutNode.operator int());
        }
        cv::FileNode initMinKfNode = fSettings["Init.MinKeyFrames"];
        if(!initMinKfNode.empty() && initMinKfNode.isInt())
        {
            mInitMinKeyFrames = initMinKfNode.operator int();
        }
        cv::FileNode initStableFramesNode = fSettings["Init.StableFrames"];
        if(!initStableFramesNode.empty() && initStableFramesNode.isInt())
        {
            mInitStableFramesRequired = initStableFramesNode.operator int();
        }

        // Optional keyframe time thresholds; defaults are kept when keys are missing.
        cv::FileNode kfNoImuInitNode = fSettings["KeyFrame.TimeThresholdNoImuInit"];
        if(!kfNoImuInitNode.empty())
        {
            if(kfNoImuInitNode.isReal())
                mKeyFrameTimeThresholdNoImuInit = kfNoImuInitNode.real();
            else if(kfNoImuInitNode.isInt())
                mKeyFrameTimeThresholdNoImuInit = static_cast<double>(kfNoImuInitNode.operator int());
        }
        cv::FileNode kfImuInitNode = fSettings["KeyFrame.TimeThresholdImuInit"];
        if(!kfImuInitNode.empty())
        {
            if(kfImuInitNode.isReal())
                mKeyFrameTimeThresholdImuInit = kfImuInitNode.real();
            else if(kfImuInitNode.isInt())
                mKeyFrameTimeThresholdImuInit = static_cast<double>(kfImuInitNode.operator int());
        }
        // Optional local mapping IMU init thresholds; defaults are kept when keys are missing.
        cv::FileNode imuInitMaxTimeNode = fSettings["LocalMapping.ImuInitMaxTime"];
        if(!imuInitMaxTimeNode.empty())
        {
            if(imuInitMaxTimeNode.isReal())
                mImuInitMaxTime = static_cast<float>(imuInitMaxTimeNode.real());
            else if(imuInitMaxTimeNode.isInt())
                mImuInitMaxTime = static_cast<float>(imuInitMaxTimeNode.operator int());
        }
        cv::FileNode imuInitMinMotionNode = fSettings["LocalMapping.ImuInitMinMotion"];
        if(!imuInitMinMotionNode.empty())
        {
            if(imuInitMinMotionNode.isReal())
                mImuInitMinMotion = static_cast<float>(imuInitMinMotionNode.real());
            else if(imuInitMinMotionNode.isInt())
                mImuInitMinMotion = static_cast<float>(imuInitMinMotionNode.operator int());
        }

        // Deeper mono-inertial initialization parameters.
        {
            cv::FileNode n = fSettings["MonoInit.TimeoutSec"];
            if (!n.empty() && (n.isReal() || n.isInt()))
                mMonoInitTimeoutSec = static_cast<float>(n.operator double());

            n = fSettings["MonoInit.MinRotDeg"];
            if (!n.empty() && (n.isReal() || n.isInt()))
                mMonoInitMinRotDeg = static_cast<float>(n.operator double());

            n = fSettings["MonoInit.MaxRotDeg"];
            if (!n.empty() && (n.isReal() || n.isInt()))
                mMonoInitMaxRotDeg = static_cast<float>(n.operator double());

            n = fSettings["MonoInit.MinDtSec"];
            if (!n.empty() && (n.isReal() || n.isInt()))
                mMonoInitMinDtSec = static_cast<float>(n.operator double());

            n = fSettings["Tracking.MinInliersNoImu"];
            if (!n.empty() && n.isInt())
                mnTrackLocalMapMinInliersNoImu = n.operator int();

            n = fSettings["Tracking.MinInliersReloc"];
            if (!n.empty() && n.isInt())
                mnTrackLocalMapMinInliersReloc = n.operator int();

            n = fSettings["MonoInit.MinTrackedMapPoints"];
            if (!n.empty() && n.isInt())
                mnMonoInitMinTrackedMapPoints = n.operator int();

            // Duration (s) the system stays in RECENTLY_LOST before transitioning to LOST.
            n = fSettings["Tracking.TimeRecentlyLost"];
            if (!n.empty() && (n.isReal() || n.isInt()))
                time_recently_lost = n.operator double();

            n = fSettings["Tracking.ResetMinKeyFrames"];
            if (!n.empty() && n.isInt())
                mnResetMinKeyFrames = n.operator int();
        }
...
{
    //f_track_stats.close();
    if(mpImuInitializerHelper)
    {
        delete mpImuInitializerHelper;
        mpImuInitializerHelper = nullptr;
    }
...
bool Tracking::IsInertialSensor() const
{
    return mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD;
}

void Tracking::ResetInitState()
{
    mInitStage = MONO_ONLY;
    mInitStartTime = -1.0;
    mInitStableFrameCount = 0;
    mbInitVioLogged = false;
}

bool Tracking::UpdateInitState(Map* pCurrentMap)
{
    if(!IsInertialSensor() || pCurrentMap == nullptr || mpImuInitializerHelper == nullptr)
    {
        return false;
    }

    if(mState == OK && !mbInitMonoLogged)
    {
        cout << "[INIT] Mono initialized" << endl;
        mbInitMonoLogged = true;
    }

    if(mState == LOST)
    {
        cout << "[RESET] Tracking lost, reinitializing" << endl;
        ResetInitState();
        return false;
    }

    if(mInitStage == MONO_ONLY && mState == OK && mpImuInitializerHelper->CanStartInitialization(mInitMinKeyFrames))
    {
        mInitStage = WAIT_IMU;
        mInitStartTime = mCurrentFrame.mTimeStamp;
        cout << "[INIT] IMU initialization started" << endl;
        return false;
    }

    if(mInitStage == WAIT_IMU)
    {
        if(mpImuInitializerHelper->IsImuInitialized())
        {
            if(mState == OK)
            {
                ++mInitStableFrameCount;
            }
            if(mInitStableFrameCount >= mInitStableFramesRequired)
            {
                mInitStage = VIO_READY;
                if(!mbInitVioLogged)
                {
                    cout << "[INIT] IMU initialization success, scale=computed" << endl;
                    cout << "[INIT] Switch to VIO" << endl;
                    mbInitVioLogged = true;
                }
            }
            return false;
        }

        if(mInitStartTime > 0.0 && (mCurrentFrame.mTimeStamp - mInitStartTime) > mInitTimeout)
        {
            cout << "[INIT] IMU init timeout, fallback to MONO_ONLY" << endl;
            ResetInitState();
    ...
    if(mInitStage == VIO_READY && !mpImuInitializerHelper->IsImuInitialized())
    {
        ResetInitState();
    }
...
void Tracking::Track()
{
    ...
    if(mState==NO_IMAGES_YET)
    {
        ...
        cout << "[TRACK] First frame received, ts=" << fixed << setprecision(3) << mCurrentFrame.mTimeStamp
             << " nKeys=" << mCurrentFrame.mvKeys.size() << endl;
    }
    ...
    if(UpdateInitState(pCurrentMap))
    {
        return;
    }
    ...
    if(mState!=NOT_INITIALIZED)
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                if((!mbVelocity && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId<mnLastRelocFrameId+2)
                {
                    ...
                    if(!bOK)
                    {
                        sTrackFailReason = "TrackReferenceKeyFrame failed in state OK without velocity or with uninitialized IMU";
                    }
                }
                else
                {
                    ...
                    if(!bOK)
                    {
                        sTrackFailReason = "TrackWithMotionModel failed in state OK, trying TrackReferenceKeyFrame";
                        bOK = TrackReferenceKeyFrame();
                        if(!bOK)
                        {
            ...
            else
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    if((mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD))
                    {
                        ...
                        {
                            bOK = false;
                            sTrackFailReason = "IMU not initialized in RECENTLY_LOST state";
                        }
                        ...
                        if (mCurrentFrame.mTimeStamp-mTimeStampLost>time_recently_lost)
                        {
                            ...
                            bOK=false;
                            sTrackFailReason = "Tracking lost for more than recently_lost threshold in RECENTLY_LOST state with IMU";
                        }
                    }
                    else
                    {
                        ...
                        if(!bOK)
                        {
                            sTrackFailReason = "Relocalization failed in RECENTLY_LOST state (visual)";
                        }
                        ...
                        if(mCurrentFrame.mTimeStamp-mTimeStampLost>3.0f && !bOK)
                        {
                            ...
                            Verbose::PrintMess("Track Lost...", Verbose::VERBOSITY_NORMAL);
                            bOK=false;
                            sTrackFailReason = "Tracking lost for more than 3.0s in RECENTLY_LOST state (visual)";
                        }
                    }
                }
                else if (mState == LOST)
                {
                    ...
                    if (pCurrentMap->KeyFramesInMap() <= mnResetMinKeyFrames)
                    {
                        mpSystem->ResetActiveMap();
        ...
        else
        {
            ...
            if(mState==LOST)
            {
                ...
                if(!bOK)
                {
                    sTrackFailReason = "Relocalization failed in localization-only mode with LOST state";
                }
            }
            else
            {
                if(!mbVO)
                {
                    ...
                    if(!bOK)
                    {
                        sTrackFailReason = "Tracking failed in localization-only mode without VO (motion model or reference KF)";
                    }
                }
                else
                {
                    ...
                    if(!bOK)
                    {
                        sTrackFailReason = "Tracking failed in localization-only mode with VO (both motion model and relocalization failed or not used)";
                    }
                }
            }
        }
        ...
        if(!mbOnlyTracking)
        {
            if(bOK)
            {
                ...
                if(!bOK)
                {
                    sTrackFailReason = "TrackLocalMap failed after pose prediction";
                }

            }
            if(!bOK)
            {
                if(!sTrackFailReason.empty())
                    cout << "Fail to track local map! Reason: " << sTrackFailReason << endl;
                else
                    cout << "Fail to track local map! Reason: unknown" << endl;
            }
        }
        else
        {
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mbReadyToInitializate)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            cout << "[MONO_INIT] reference frame set, nKeys=" << mCurrentFrame.mvKeys.size()
                 << " ts=" << fixed << setprecision(3) << mCurrentFrame.mTimeStamp << endl;
        ...
        {
            // Print once every ~50 frames to avoid flooding
            static int sNoFeatCount = 0;
            if((++sNoFeatCount % 50) == 1)
                cout << "[MONO_INIT] waiting for reference frame, nKeys=" << mCurrentFrame.mvKeys.size() << endl;
        }
    }
    else
    {
        ...
        {
            cout << "[MONO_INIT] reset: too few keypoints=" << mCurrentFrame.mvKeys.size() << endl;
            mbReadyToInitializate = false;
            return;
        }
        // Use configurable timeout (default 3.0s) instead of hardcoded 1.0s.
        // Longer window lets the camera accumulate enough rotation even during forward driving.
        if((mSensor == System::IMU_MONOCULAR)&&(mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp>mMonoInitTimeoutSec))
        {
            cout << "[MONO_INIT] reset: timeout dt=" << fixed << setprecision(3)
                 << (mLastFrame.mTimeStamp-mInitialFrame.mTimeStamp) << "s"
                 << " (limit=" << mMonoInitTimeoutSec << "s)" << endl;
            mbReadyToInitializate = false;
            return;
        }

        // IMU rotation gate + dt baseline gate.
        // - Skip if rotation < mMonoInitMinRotDeg: near-pure-translation → degenerate F/H.
        // - Skip if rotation > mMonoInitMaxRotDeg: rotation-dominated with minimal translation
        //   (e.g. tight turn at low speed) → triangulation degrades due to near-zero baseline.
        // - Skip if dt < mMonoInitMinDtSec: baseline too short → large depth uncertainty.
        if (mSensor == System::IMU_MONOCULAR && mpImuPreintegratedFromLastKF != nullptr)
        {
            const Eigen::Matrix3f dR = mpImuPreintegratedFromLastKF->GetUpdatedDeltaRotation();
            const float cosAngle = (dR.trace() - 1.0f) / 2.0f;
            const float rotDeg = std::acos(std::max(-1.0f, std::min(1.0f, cosAngle)))
                                 * 180.0f / static_cast<float>(M_PI);
            const double dt = mCurrentFrame.mTimeStamp - mInitialFrame.mTimeStamp;

            if (mMonoInitMinRotDeg > 0.0f && rotDeg < mMonoInitMinRotDeg)
            {
                static int sRotLowSkip = 0;
                if ((++sRotLowSkip % 30) == 1)
                    cout << "[MONO_INIT] rot gate: " << fixed << setprecision(2)
                         << rotDeg << "deg < " << mMonoInitMinRotDeg
                         << "deg threshold (dt=" << setprecision(3) << dt << "s), waiting" << endl;
                return;
            }
            if (mMonoInitMaxRotDeg > 0.0f && rotDeg > mMonoInitMaxRotDeg)
            {
                static int sRotHighSkip = 0;
                if ((++sRotHighSkip % 15) == 1)
                    cout << "[MONO_INIT] rot gate: " << fixed << setprecision(2)
                         << rotDeg << "deg > " << mMonoInitMaxRotDeg
                         << "deg (rotation-dominated, dt=" << setprecision(3) << dt << "s), skipping" << endl;
                return;
            }
            if (mMonoInitMinDtSec > 0.0f && static_cast<float>(dt) < mMonoInitMinDtSec)
            {
                // Baseline too short: triangulated 3D points would have huge depth uncertainty.
                return;
            }
            cout << "[MONO_INIT] rot gate passed: " << fixed << setprecision(2)
                 << rotDeg << "deg >= " << mMonoInitMinRotDeg
                 << "deg, attempting recon (dt=" << setprecision(3) << dt << "s)" << endl;
        }
        ...
        if(nmatches<100)
        {
            cout << "[MONO_INIT] reset: not enough matches=" << nmatches << "/100" << endl;
            mbReadyToInitializate = false;
            return;
        }
        ...
        {
            cout << "[MONO_INIT] ReconstructWithTwoViews failed, nmatches=" << nmatches
                 << " dt=" << fixed << setprecision(3)
                 << (mCurrentFrame.mTimeStamp-mInitialFrame.mTimeStamp) << "s" << endl;
        }
    }
}
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // Threshold is configurable (MonoInit.MinTrackedMapPoints, default 30).
    if(medianDepth<0 || pKFcur->TrackedMapPoints(1)<mnMonoInitMinTrackedMapPoints)
    {
        cout << "[MONO_INIT] CreateInitialMap rejected: medianDepth=" << fixed << setprecision(3)
             << medianDepth << " trackedPts=" << pKFcur->TrackedMapPoints(1)
             << " (min=" << mnMonoInitMinTrackedMapPoints << "), resetting" << endl;
...
{
    // Homography-based rotation recovery.
    // Called when both TrackWithMotionModel and TrackReferenceKeyFrame fail.
    // Useful for near-pure rotation (ground robot turning) where the constant-
    // velocity model overshoots and the reference KF view is too different.
    // Works purely from 2D-2D correspondences — no pose prior needed.  (but can we use some pose priors just right before it turns?)

    //Match current frame against last frame using spatial window
    ORBmatcher matcher(0.9, true);
    vector<cv::Point2f> vbPrevMatched(mLastFrame.mvKeysUn.size());
    for(size_t i = 0; i < mLastFrame.mvKeysUn.size(); i++)
        vbPrevMatched[i] = mLastFrame.mvKeysUn[i].pt;

    vector<int> vnMatches12(mLastFrame.mvKeysUn.size(), -1);
    int nmatches = matcher.SearchForInitialization(mLastFrame, mCurrentFrame,
                                                   vbPrevMatched, vnMatches12, 100);
    if(nmatches < 30)
    {
        cout << "TRACK_H: Not enough 2D matches (" << nmatches << ")\n";
        return false;
    }

    //Estimate relative pose via H/F (H wins for planar/pure-rotation scenes).   // same as the init logic, when init, there is a H/F tradeoff, while F only when the SLAM is running, I think the adaptive H/F selection is important for monocular SLAM
    Sophus::SE3f T21;
    vector<cv::Point3f> vP3D;
    vector<bool> vbTriangulated;
    if(!mpCamera->ReconstructWithTwoViews(mLastFrame.mvKeysUn, mCurrentFrame.mvKeysUn,
                                          vnMatches12, T21, vP3D, vbTriangulated))
    {
        cout << "TRACK_H: ReconstructWithTwoViews failed\n";
        return false;
    }

    //Build corrected initial pose:
    //Rotation  — from H decomposition (correct even for pure rotation)
    //Translation — scale H unit-vector by motion model magnitude (or zero if no velocity)
    Eigen::Matrix3f R_hl = T21.rotationMatrix();
    Eigen::Vector3f t_hl;
    if(mbVelocity)
    {
        Sophus::SE3f T_mm = mVelocity * mLastFrame.GetPose();
        float t_scale = T_mm.translation().norm();
        t_hl = T21.translation() * t_scale;
    }
    else
    {
        t_hl = Eigen::Vector3f::Zero();
    }
    mCurrentFrame.SetPose(Sophus::SE3f(R_hl, t_hl) * mLastFrame.GetPose());

    //SearchByProjection from the H-corrected initial pose
    fill(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(),
         static_cast<MapPoint*>(NULL));

    int th = 25;
    int nMatchesProj = matcher.SearchByProjection(mCurrentFrame, mLastFrame, th, true);
    if(nMatchesProj < 20)
    {
        fill(mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(),
             static_cast<MapPoint*>(NULL));
        nMatchesProj = matcher.SearchByProjection(mCurrentFrame, mLastFrame, 2*th, true);
    }
    if(nMatchesProj < 20)
    {
        cout << "TRACK_H: Not enough projection matches (" << nMatchesProj << ")\n";
        return false;
    }
    ...
    //Count inliers (same pattern as TrackWithMotionModel)
    int nmatchesMap = 0;
    for(int i = 0; i < mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
    ...
    if(nmatchesMap < 10)
    {
        cout << "TRACK_H: PoseOptimization inliers too few (" << nmatchesMap << ")\n";
        return false;
    }

    cout << "TRACK_H: Success — " << nmatchesMap << " inliers\n";
    return true;
}
...
bool Tracking::TrackLocalMap()
{
    ...
    // Configurable via Tracking.MinInliersReloc (default 30, was hardcoded 40).
    if(mCurrentFrame.mnId<mnLastRelocFrameId+mMaxFrames && mnMatchesInliers<mnTrackLocalMapMinInliersReloc)
    {
        cout << "TrackLocalMap: fail (recently relocalized). "
             << "mnId=" << mCurrentFrame.mnId
             << ", mnLastRelocFrameId=" << mnLastRelocFrameId
             << ", mMaxFrames=" << mMaxFrames
             << ", mnMatchesInliers=" << mnMatchesInliers
             << ", threshold=" << mnTrackLocalMapMinInliersReloc
             << endl;
        return false;
    }
    ...
    if (mSensor == System::IMU_MONOCULAR)
    {
        const bool bImuInit = mpAtlas->isImuInitialized();
        // Pre-IMU-init threshold is configurable (Tracking.MinInliersNoImu, default 30).
        const int thresh = bImuInit ? 15 : mnTrackLocalMapMinInliersNoImu;
        if(mnMatchesInliers < thresh)
        {
            cout << "TrackLocalMap: fail (IMU_MONOCULAR). "
                 << "imuInitialized=" << (bImuInit ? "true" : "false")
                 << ", mnMatchesInliers=" << mnMatchesInliers
                 << ", threshold=" << thresh
                 << endl;
            return false;
    ...
    else if (mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
    {
        ...
        {
            cout << "TrackLocalMap: fail (IMU_STEREO/IMU_RGBD). "
                 << "mnMatchesInliers=" << mnMatchesInliers
                 << ", threshold=15" << endl;
            return false;
        }
    ...
    else
    {
        ...
        {
            cout << "TrackLocalMap: fail (pure visual). "
                 << "mnMatchesInliers=" << mnMatchesInliers
                 << ", threshold=30" << endl;
            return false;
        }
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    {
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=mKeyFrameTimeThresholdNoImuInit)
            return true;
        else if ((mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD) && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=mKeyFrameTimeThresholdNoImuInit)
            return true;
        else
            return false;
    }
    ...
    if(mpLastKeyFrame)
    {
        if (mSensor==System::IMU_MONOCULAR)
        {
            if ((mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=mKeyFrameTimeThresholdImuInit)
                c3 = true;
        }
        else if (mSensor==System::IMU_STEREO || mSensor == System::IMU_RGBD)
        {
